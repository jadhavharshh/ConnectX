...
exports.buildCourseEmail = exports.buildTaskEmail = exports.buildAnnouncementEmail = void 0;
...
const renderParagraphs = (lines) => lines
    .filter(Boolean)
    .map((line) => `<p style="margin: 0 0 14px 0; color: #0f172a; font-size: 16px; line-height: 1.7;">${line}</p>`)
    .join("");
const renderDetails = (details) => {
    ...
    const rows = details
        ...
        return `
        <tr>
          <td style="padding: 8px 0; font-weight: 600; color: #0f172a; font-size: 13px; letter-spacing: 0.4px;">${htmlEscape(detail.label)}</td>
          <td style="padding: 8px 0; font-size: 13px; color: #475569;">${htmlEscape(detail.value || "-")}</td>
        </tr>`;
    })
        .join("");
    return `
...
const renderBaseTemplate = ({ headline, title, introLines, highlightLabel, highlightValue, details, cta, footerNote, preheader, }) => {
    ...
    <body style="margin: 0; background-color: #0f172a; font-family: 'Inter', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #0f172a;">
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="width: 100%; padding: 40px 12px;">
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" style="width: 100%; max-width: 640px; background: linear-gradient(145deg, #f8fafc 0%, #f1f5f9 100%); border-radius: 28px; overflow: hidden; border: 1px solid rgba(148, 163, 184, 0.25); box-shadow: 0 40px 80px rgba(15, 23, 42, 0.20);">
              <tr>
                <td style="padding: 40px 42px 30px 42px; background: radial-gradient(circle at 0% 0%, rgba(37, 99, 235, 0.18), transparent 55%), radial-gradient(circle at 100% 0%, rgba(14, 165, 233, 0.22), transparent 62%);">
                  <p style="margin: 0; letter-spacing: 4px; font-size: 12px; text-transform: uppercase; color: rgba(15, 23, 42, 0.55); font-weight: 600;">${htmlEscape(headline)}</p>
                  <h1 style="margin: 16px 0 0 0; font-size: 32px; font-weight: 700; color: #0f172a; letter-spacing: -0.6px;">${htmlEscape(title)}</h1>
                </td>
              </tr>
              <tr>
        ...
        ? `<div style="display: inline-flex; align-items: center; padding: 12px 18px; margin-top: -26px; border-radius: 999px; background: linear-gradient(135deg, rgba(37, 99, 235, 0.08), rgba(14, 165, 233, 0.12)); border: 1px solid rgba(59, 130, 246, 0.20); box-shadow: 0 15px 30px rgba(37, 99, 235, 0.10);">
                        <span style="font-size: 13px; text-transform: uppercase; color: rgba(15, 23, 42, 0.55); letter-spacing: 2px; font-weight: 600; margin-right: 14px;">${htmlEscape(highlightLabel)}</span>
                        <span style="font-size: 15px; font-weight: 600; color: #0f172a;">${htmlEscape(highlightValue)}</span>
                      </div>`
        : ""}
...
const buildAnnouncementEmail = (options) => {
...
exports.buildAnnouncementEmail = buildAnnouncementEmail;
const buildTaskEmail = (options) => {
...
exports.buildTaskEmail = buildTaskEmail;
const buildCourseEmail = (options) => {
    ...
    const highlightMap = {
        ...
        "module-added": {
            label: "Module Added",
            value: (module === null || module === void 0 ? void 0 : module.title) || "New Module",
        },
        "lesson-added": {
            label: "Lesson Added",
            value: (lesson === null || lesson === void 0 ? void 0 : lesson.title) || "New Lesson",
        },
        "lesson-updated": {
            label: "Lesson Updated",
            value: (lesson === null || lesson === void 0 ? void 0 : lesson.title) || "Lesson",
        },
    };
    return renderBaseTemplate({
...
exports.buildCourseEmail = buildCourseEmail;
